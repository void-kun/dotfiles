...
;;; lolo-funcs.el --- xxx.	-*- lexical-binding: t -*-
...
(defun find-custom-file()
    ...
    (unless (file-exists-p custom-file)
            (if (file-exists-p lolo-custom-file)
                (copy-file lolo-custom-file custom-file)
                (user-error "The file `%s' doesn't exist" lolo-custom-file)))
    (when (file-exists-p custom-file)
          (find-file custom-file))
    (when (file-exists-p lolo-post-file)
          (find-file-other-window lolo-post-file)))
...
(defun update-config-and-packages()
    ...
    (update-config)
    (update-packages))
(defalias 'lolo-update #'update-config-and-packages)
...
(defalias 'lolo-update-org #'update-org)

(defun update-all()
    "Update org files, configurations and packages to the latest."
    (interactive)
    (update-org)
    (update-config-and-packages))
(defalias 'lolo-update-all #'update-all)
...
(defun childframe-completion-workable-p ()
    ...
    (and (eq lolo-completion-style 'childframe)
        (childframe-workable-p)))

(defun lolo--theme-name (theme)
    "Return internal THEME name."
    (or (alist-get theme lolo-theme-alist) theme 'monokai-pro))

(defun lolo-compatible-theme-p (theme)
    "Check if the THEME is compatible. THEME is a symbol."
    (or (memq theme '(auto random system))
        (string-prefix-p "monokai" (symbol-name (lolo--theme-name theme)))))

(defun lolo-dark-theme-p ()
    "Check if the current theme is a dark theme."
    (eq (frame-parameter nil 'background-mode) 'dark))

(defun lolo-theme-enable-p (theme)
    "The THEME is enabled or not."
    (and theme
        (not (memq lolo-theme '(auto random system)))
        (memq (lolo--theme-name theme) custom-enabled-themes)))

(defun lolo--load-theme (theme)
    "Disable others and enable new THEME."
    (when-let ((theme (lolo--theme-name theme)))
        (mapc #'disable-theme custom-enabled-themes)
        (load-theme theme t)))

(defun lolo--load-system-theme (appearance)
    "Load theme, taking current system APPEARANCE into consideration."
    (lolo--load-theme (alist-get appearance lolo-system-themes)))

(defun lolo-load-random-theme ()
    "Load the random theme."
    (interactive)
    (let* ((themes (mapcar #'cdr lolo-theme-alist))
            (theme (nth (random (length themes)) themes)))
        (if (eq theme lolo-theme)
            (lolo-load-random-theme)
        (lolo--load-theme theme))))

(defun lolo-load-theme (theme &optional no-save)
  ...
  (pcase theme
    ('auto
     ...
     (use-package circadian
       ...
       :custom (circadian-themes lolo-auto-themes)
  ...
  ;; Set option
  (lolo-set-variable 'lolo-theme theme no-save))

(advice-add #'consult-theme :after
    (lambda (theme)
        "Save theme."
        (lolo-set-variable 'lolo-theme theme)))
