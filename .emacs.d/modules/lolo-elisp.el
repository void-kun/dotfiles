;;; lolo-elisp.el --- Zrik's Emacs setup.  -*- lexical-binding: t; -*-
;;
;;; Commentary:
;;
;;
   ...
   (defun my-lisp-indent-function (indent-point state)
     ...
     (let ((normal-indent (current-column))
       ...
       (cond
        ...
        ((and (elt state 2)
              (or (not (looking-at "\\sw\\|\\s_")) (looking-at ":")))
         (if (not
              (> (save-excursion
                   (forward-line 1)
                   (point))
       ...
       (save-excursion
         (when-let ((hook
                     (progn
                       ...
                       (symbol-at-point)))
                    (func
                     (when (and (or (re-search-forward (format
                                                        "^Value:?[\s|\n]")
                                                       nil t)
                                    (goto-char orig-point))
                                (sexp-at-point))
...
(use-package
 ...
 :bind
 (:map
  emacs-lisp-mode-map ("C-c e" . macrostep-expand)
 ...
 (with-no-warnings
   ...
   (defun my-helpful--navigate (button)
     ...
     (find-file-other-window
      (substring-no-properties (button-get button 'path)))
     ;; We use `get-text-property' to work around an Emacs 25 bug:
     ;; http://git.savannah.gnu.org/cgit/emacs.git/commit/?id=f7c4bad17d83297ee9a1b57552b1944020f23aea
     (-when-let (pos
                 (get-text-property button 'position (marker-buffer button)))
       (helpful--goto-char-widen pos)))
   (advice-add #'helpful--navigate :override #'my-helpful--navigate)))

;; Integrate Ert-runner
(use-package overseer :diminish :hook (emacs-lisp-mode . overseer-mode))


(provide 'lolo-elisp)
;;; lolo-elisp.el ends here
