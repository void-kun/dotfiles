     ...
     :hook ((prog-mode . (lambda ()
            ...
            (lsp-mode . (lambda ()
                          ...
                          (when (and lolo-lsp-format-on-save
                                     (not (apply #'derived-mode-p lolo-lsp-format-on-save-ignore-modes)))
...
(when lolo-lsp
           ...
           (pcase lolo-lsp
             ...
             ('lsp-mode
              (when (fboundp 'lsp-deferred)
                ...
                (lsp-deferred)))
             (_
              (user-error "LSP:: invalid `lolo-lsp' type"))))
         (put ',intern-pre 'function-documentation
              (format "Enable `%s' in the buffer of org source block (%s)."
                      lolo-lsp (upcase ,lang)))
  ...
  (defconst org-babel-lang-list
    '("go" "python" "ipython" "js" "css" "sass" "c" "rust" "java" "cpp" "c++" "shell")
