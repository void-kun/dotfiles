;;; lolo-highlight.el --- Zrik's Emacs setup.  -*- lexical-binding: t; -*-
;;
;;; Commentary:
;;
;;
     ...
     (when (and (overlay-buffer show-paren--overlay)
                (not
                 (or cursor-in-echo-area
                     ...
                     (minibufferp)
                     this-command))
                (and (not (bobp)) (memq (char-syntax (char-before)) '(?\) ?\$)))
                (= 1
                   (logand
                    1
                    (- (point)
...
(use-package
 ...
 (symbol-overlay-default-face
  ((t (:inherit region :background unspecified :foreground unspecified))))
   ...
   (defun my-rainbow-colorize-match (color &optional match)
     (let* ((match (or match 0))
       ...
       (overlay-put
        ov 'face
        `((:foreground
           ,(if (> 0.5 (rainbow-x-color-luminance color))
                "white"
              "black"))
          (:background ,color)))))
...
(use-package
 ...
 (defun hl-todo-rg-project ()
   ...
   (unless (require 'rg nil t)
     (error "`rg' is not installed"))
   (rg-project
    (replace-regexp-in-string "\\\\[<>]*" "" (hl-todo--regexp)) "everything")))
...
(use-package
 ...
 (diff-hl-change
  ((t
    (:inherit custom-changed :foreground unspecified :background unspecified))))
 (diff-hl-insert ((t (:inherit diff-added :background unspecified))))
 (diff-hl-delete ((t (:inherit diff-removed :background unspecified))))
 :bind (:map diff-hl-command-map ("SPC" . diff-hl-mark-hunk))
 :hook
...
(use-package
 ...
 (pulse-highlight-face
  ((t (:inherit region :background unspecified :extend t))))
 :hook
 (((dumb-jump-after-jump imenu-after-jump) . my-recenter-and-pulse)
  ((bookmark-after-jump magit-diff-visit-file next-error)
   .
...
(use-package goggles :diminish :hook ((prog-mode text-mode) . goggles-mode))

(provide 'lolo-highlight)
;;; lolo-highlight.el ends here
