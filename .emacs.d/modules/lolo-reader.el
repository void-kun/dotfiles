;;; lolo-reader.el --- Zrik's Emacs setup.  -*- lexical-binding: t; -*-
;;
;;; Commentary:
;;
;;
  ...
  (use-package
   ...
   (use-package
    ...
    (advice-add
     ...
     :around #'saveplace-pdf-view-find-file-advice)
    (advice-add
     'save-place-to-alist
     :around #'saveplace-pdf-view-to-alist-advice))))
 ...
 (with-no-warnings
   ...
   (defun my-nov-content-unique-identifier (content)
     ...
     (let* ((name (nov-content-unique-identifier-name content))
            (selector
             (format "package>metadata>identifier[id='%s']"
                     (regexp-quote name)))
            (id (car (esxml-node-children (esxml-query selector content)))))
       (and id (intern id))))
   (advice-add
    #'nov-content-unique-identifier
...
(use-package
 ...
 ((:title
   ...
   :color amaranth
   :quit-key ("q" "C-g"))
  ("Search" (("c" elfeed-db-compact "compact db")
 ...
 (("C-x w" . elfeed)
  ...
  ("q" . delete-window))
 :hook (elfeed-show-mode . lolo-read-mode)
 :init
 (setq
 ...
 (when (icons-displayable-p)
  ...
  Returns default if no match."
     (cond
      ...
      ((or (member "emacs" tags)
           (member "emacslife" tags)
           (member "mastering" tags))
   ...
   (defun lucius/elfeed-search-print-entry--better-default (entry)
     ...
     (let* ((date (elfeed-search-format-date (elfeed-entry-date entry)))
       ...
       (put-text-property
        (1- (point)) (point) 'display `(space :align-to ,align-to-feed-pixel))
       ;; (when feed-title (insert " " (propertize feed-title 'face 'elfeed-search-feed-face) " "))
       (when feed-title
         (insert
          " "
          (concat (nerd-icon-for-tags tags) " ")
   ...
   (setq elfeed-search-print-entry-function
         #'lucius/elfeed-search-print-entry--better-default))
...
browser defined by `browse-url-generic-program'."
   ...
   (advice-add
    ...
    :override #'my-elfeed-search-browse-url)))
...
(use-package
 ...
 :hook (newsticker-treeview-item-mode . lolo-read-mode)
 :init
