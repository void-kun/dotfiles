;;; lolo-edit.el --- Zrik's Emacs setup.  -*- lexical-binding: t; -*-
;;
;;; Commentary:
;;
;;
...
(use-package
 ...
 :bind
 (:map
  text-mode-map ("<C-return>" . rect-hydra/body)
  :map prog-mode-map ("<C-return>" . rect-hydra/body))
 :init
 ...
 :pretty-hydra
 ((:title
   (pretty-hydra-title "Rectangle" 'mdicon "nf-md-border_all")
   :color amaranth
   :body-pre (rectangle-mark-mode)
   :post (deactivate-mark)
   :quit-key ("q" "C-g"))
  ("Move" (("h" backward-char "←")
    ("j" next-line "↓")
    ("k" previous-line "↑")
    ("l" forward-char "→"))
   "Action"
...
(use-package
 ...
 (add-to-list 'aggressive-indent-protected-commands #'delete-trailing-whitespace
              t)
 ...
 (add-to-list
  ...
  '(and (derived-mode-p
         'c-mode 'c++-mode 'csharp-mode 'java-mode 'go-mode 'swift-mode)
        (null
         (string-match
          "\\([;{}]\\|\\b\\(if\\|for\\|while\\)\\b\\)"
          (thing-at-point 'line))))))
...
(use-package
 ...
 :config (mapc (lambda (pair) (diminish (cdr pair))) beginend-modes))
...
(use-package
 ...
 :pretty-hydra
 ((:title
   (pretty-hydra-title "Multiple Cursors" 'mdicon "nf-md-cursor_move")
   :color amaranth
   :quit-key ("q" "C-g"))
  ("Up" (("p" mc/mark-previous-like-this "prev")
...
(use-package smart-region :hook (after-init . smart-region-on))
...
(use-package
 ...
 (setq
  ...
  hungry-delete-except-modes '(help-mode minibuffer-mode minibuffer-inactive-mode calc-mode)))
...
(use-package
 ...
 :ensure nil
 :diminish
 :hook ((prog-mode . subword-mode) (minibuffer-setup . subword-mode)))
...
(use-package
 ...
 :pretty-hydra
 ((:title
   (pretty-hydra-title "HideShow" 'octicon "nf-oct-fold")
   :color amaranth
   :quit-key ("q" "C-g"))
  ("Fold" (("t" hs-toggle-all "toggle all")
 ...
 (defun hs-toggle-all ()
   ...
   (pcase last-command
     ('hs-toggle-all
      (save-excursion (hs-show-all)) (setq this-command 'hs-global-show))
     (_ (hs-hide-all))))
...
;; Narrow/Widen
(use-package fancy-narrow :diminish :hook (after-init . fancy-narrow-mode))

;; Hanlde minified code
(use-package so-long :hook (after-init . global-so-long-mode))

(provide 'lolo-edit)
;;; lolo-edit.el ends here
