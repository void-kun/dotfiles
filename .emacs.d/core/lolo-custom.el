...
(defcustom lolo-package-archives-alist
  ...
  :type
  '(alist
    :key-type (symbol :tag "Archive group name")
    :value-type
    (alist
     :key-type (string :tag "Archive name")
...
(defcustom lolo-package-archives 'melpa
  ...
  `(choice
    ...
    (mapcar
     (lambda (item)
       (let ((name (car item)))
         (list 'const :tag (capitalize (symbol-name name)) name)))
...
(defcustom lolo-lsp-format-on-save t
