...
(lolo-install-search-engine "github"    "http://github.com/search?q="                   "Search Github: ")

(defun lolo-recompile-init ()
    "Byte-compile all your dotfiles agains."
    (interactive)
...
(defun lolo-tip-of-the-day ()
    ...
    (when (and lolo-tips (not (window-minibuffer-p)))
        ...
        (random t)
        (message
        (concat "tip: " (nth (random (length lolo-tips)) lolo-tips)))))
...
(defun lolo-update-packages (&optional arg)
    "Update Lolo's packages"
    (interactive "P")
    (when (y-or-n-p "Do you want to update Lolo's packages? ")
        (if arg (epl-upgrade)
            (epl-upgrade (cl-remove-if-not (lambda (p) (memq (epl-package-name p) lolo-packages))
