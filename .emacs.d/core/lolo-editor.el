...
(defun lolo-enable-flyspell ()
  "Enable command `flyspell-mode' if `lolo-flyspell' is not nil."
  (when (and lolo-flyspell (executable-find ispell-program-name))
    (flyspell-mode +1)))

(defun lolo-cleanup-maybe ()
  "Invoke `whitespace-cleanup' if `lolo-clean-whitespace-on-save' is not nil."
  (when lolo-clean-whitespace-on-save
    (whitespace-cleanup)))

(defun lolo-enable-whitespace ()
  "Enable `whitespace-mode' if `lolo-whitespace' is not nil."
...
indent yanked text (with prefix arg don't indent)."
  (if (and (not (ad-get-arg 0))
           ...
           (or (derived-mode-p 'prog-mode)
               (member major-mode lolo-yank-indent-modes)))
...
(set-face-attribute 'default nil :font "IosevkaLyteTerm" :height 120)
