...
(defcustom tabby-token nil
  "Token to access Tabby API."
  :type 'string
  :group 'tabby)
...
(defun tabby--completions-url ()
  ...
  (format "%s/v1/completions"
          (string-remove-suffix "/" tabby-api-url)))
...
When the request completes, CALLBACK will be invoked with the response."
  (let* ((request (tabby--completions-request lang prefix suffix))
	    ...
	    ("Authorization" . ,tabby-token)))
         (url-request-data (json-encode request)))
    (url-retrieve
     (tabby--completions-url)
     (lambda (_status)
...
(defun tabby-complete ()
  ...
  (when (not tabby-api-url)
    (error
     "Please configure the URL for your Tabby server. See customizable variable `tabby-api-url`"))
  (let* ((lang (tabby--determine-language))
    ...
    (if lang
        (tabby--get-completions
         (current-buffer)
         lang
         prefix
         suffix
         'tabby--handle-completion-response)
