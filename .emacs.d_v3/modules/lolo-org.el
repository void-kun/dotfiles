...
(use-package
 ...
 ((:title
  ...
  ("Basic" (("a" (hot-expand "<a") "ascii")
   ...
   (("u" (hot-expand "<s" "plantuml :file CHANGE.png") "plantuml")
    ("Y" (hot-expand
      "<s" "ipython :session :exports both :results raw drawer\n$0")
     "ipython")
    ("P" (progn
       (insert
 ...
 :bind
 (("C-c a" . org-agenda)
  ("C-c b" . org-switchb) ("C-c x" . org-capture)
  :map org-mode-map
  ("<" .
   (lambda ()
     "Insert org template."
...
prepended to the element after the #+HEADER: tag."
 ...
 (setq
  ...
  org-directory lolo-org-directory
  ...
  `(("i"
    ...
    ("b"
     "Book"
     entry
     (file+olp+datetree ,(concat org-directory "/book.org"))
     "* Topic: %^{Description}  %^g %? Added: %U"))

  org-todo-keywords
  '((sequence
     "TODO(t)" "DOING(i)" "HANGUP(h)" "|" "DONE(d)" "CANCEL(c)")
    (sequence "⚑(T)" "🏴(I)" "❓(H)" "|" "✔(D)" "✘(C)"))
  org-todo-keyword-faces '(("HANGUP" . warning) ("❓" . warning))
  org-priority-faces '((?A . error) (?B . warning) (?C . success))

  ;; Agenda styling
  org-agenda-files (list lolo-org-directory)
 ...
 (use-package ox-gfm :init (add-to-list 'org-export-backends 'gfm))
 ...
 (use-package
  ...
  :bind
  (:map
   message-mode-map ("C-c M-o" . org-mime-htmlize)
...
(use-package
 ...
 :init
 (setq
  org-roam-directory (file-truename lolo-org-directory)
 ...
 (add-to-list
  'org-agenda-files (format "%s/%s" org-roam-directory "roam"))
